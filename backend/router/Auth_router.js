...
const {verifyJWT, isAdmin, isNormalUser} = require('../middelware/verifyJWT'); 
...
router.post('/register', async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign(
      { id: newUser[0].id, email:newUser[0].email, name:newUser[0].name , role: newUser[0].role },
      jwt_secreat,
      { expiresIn: '1d' }
    );
    const success = true;
    return res.status(200).json({ token, success,msg: "Regestration succesfull" });
...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({ token, role: user[0].role,success, msg: "Login Succesfull" });
